 ...
 */
abstract class AbstractListRadixTrie<T> extends AbstractRadixTrie<T> {
	...
	static final class ArrayRadixTrieCursor<T> extends AbstractCursor<T> implements RadixTrie.Cursor<T> {
		...
		final AbstractListRadixTrie<T> trie;
		final IntArrayList parents;
		int parent, child, offset;

		protected ArrayRadixTrieCursor(AbstractListRadixTrie<T> trie, int parent, int child, int offset,
		...
		public ArrayRadixTrieCursor(AbstractListRadixTrie<T> trie) {
		...
		public boolean moveToChild(char c) {
			if (offset == 0) {
				child = trie.firstChildRadix(parent, c);
				if (child != NOT_FOUND) {
					moveToChildAux(c);
					return true;
				}
	...
	public AbstractListRadixTrie(int initialCapacity, GrowthStrategy growthStrategy) {
		this(initialCapacity, growthStrategy, 1, new Object[initialCapacity],
				new int[initialCapacity]);
		Arrays.fill(brothers, NOT_FOUND);
	}

	protected AbstractListRadixTrie(int initialCapacity, GrowthStrategy growthStrategy, int size,
	...
	protected int firstChildRadix(int position, char label) {
	...
	protected int getNode(char[] buffer, int offset, int length) {
		int node = START;
		while (length > 0) {
			node = firstChildRadix(node, buffer[offset]);
			if (node != NOT_FOUND) {
	...
	protected int getNode(CharSequence sequence, int offset, int length) {
		int node = START;
		while (length > 0) {
			node = firstChildRadix(node, sequence.charAt(offset));
			if (node != NOT_FOUND) {
				int otherCharsLength = otherLabelCharsLength(node);
				if (otherCharsLength >= length) {
					return NOT_FOUND;
	...
	protected int newNode() {
		...
		if (size >= capacity) {
			ensureCapacity(growthStrategy.grow(capacity));
		}
		return size++;
	}
