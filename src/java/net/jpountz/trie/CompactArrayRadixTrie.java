...
public class CompactArrayRadixTrie<T> extends ArrayRadixTrie<T> implements Trie.Optimizable, Trie.Trimmable, RadixTrie.LabelsInternable {

	protected int[] children;
	protected char[][] labels;

	protected CompactArrayRadixTrie(int initialCapacity, float growthFactor,
			int size, Object[] values, int[] brothers) {
		super(initialCapacity, growthFactor, size, values, brothers);
	}

	public CompactArrayRadixTrie(int initialCapacity, float growthFactor) {
	...
	public void optimizeFor(Trie.Traversal traversal) {
		CompactArrayRadixTrie<T> trie = new CompactArrayRadixTrie<T>(radixSize(), growthFactor);
